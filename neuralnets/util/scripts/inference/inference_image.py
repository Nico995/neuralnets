...
if __name__ == '__main__':
    ...
    labels = torch.tensor(tifffile.imread(params['labels'])).unbind(dim=0)

    for pred, label in zip(slides, labels):
        label = np.array(label)
        print(pred.shape)
        print(pred[pred.nonzero()])
        plt.imshow(pred)
        plt.savefig('prova_pred')
        print(label.shape)
        print(label[label.nonzero()])
        plt.imshow(label)
        plt.savefig('prova_label')

        image = overlay(pred, np.array(label))
        print(image.shape)
        print(image[image.nonzero()])
        plt.imshow(image[:,:,:,0]*255)
        plt.savefig('prova_segmap')
        exit()
